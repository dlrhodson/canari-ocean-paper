...
def plot_atlantic_ht_26n(ax):
    dir='/home/users/dlrhodso/CANARI/SPRINT_2024/analysis/Heat_Transport/'
    # 1. Load ensemble data files for `SOPHTADV_ATL_26_0_ANN` and `SOPHTLDF_ATL_26_0_ANN`

    data=cf.read(dir+'Atl_HT_26.0/Atl_HT_26.0_*_sophtadv_atl.nc')
    ...
    advective=data.select_by_ncvar("sophtadv_atl_26_0_ann")
    diffusive=data.select_by_ncvar("sophtldf_atl_26_0_ann")

    total_26_ann_list = cf.FieldList()
    slope_list = []
    for member in advective:

        sophtadv = member.squeeze()
        ...
        sophtldf = this_ldf[0].squeeze()
        total_26_ann = sophtadv + sophtldf
    ...
    #min_slope_member=total_26_ann_list[smin]
    #max_slope_member=total_26_ann_list[smax]
    ...
    #ax.xaxis.set_major_formatter(plt.FuncFormatter(lambda x, _: f"{int(x)}" if x in major_ticks else ""))
